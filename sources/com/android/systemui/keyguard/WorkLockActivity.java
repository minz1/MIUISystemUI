...
public class WorkLockActivity extends Activity {
    ...
    private void showConfirmCredentialActivity() {
        if (!isFinishing() && getKeyguardManager().isDeviceLocked(getTargetUserId())) {
            Intent credential = getKeyguardManager().createConfirmDeviceCredentialIntent(null, null, getTargetUserId());
            if (credential != null) {
                ActivityOptions options = ActivityOptions.makeBasic();
                options.setLaunchTaskId(getTaskId());
                credential.putExtra("android.intent.extra.INTENT", PendingIntent.getActivity(this, -1, getIntent(), 1409286144, options.toBundle()).getIntentSender());
                try {
                    ActivityManagerCompat.getService().startConfirmDeviceCredentialIntent(credential, getChallengeOptions().toBundle());
