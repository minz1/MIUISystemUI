...
public class QSTileView extends QSTileBaseView {
    ...
    public void handleStateChanged(QSTile.State state) {
        ...
        boolean dualTarget = state.dualTarget;
        this.mExpandIndicator.setVisibility(dualTarget ? 0 : 8);
        this.mExpandSpace.setVisibility(dualTarget ? 0 : 8);
        this.mLabelContainer.setImportantForAccessibility(dualTarget ? 1 : 2);
        ViewGroup viewGroup = this.mLabelContainer;
        if (dualTarget) {
            charSequence = state.dualLabelContentDescription;
        } else {
            charSequence = null;
        }
        viewGroup.setContentDescription(charSequence);
        if (dualTarget != this.mLabelContainer.isClickable()) {
            this.mLabelContainer.setClickable(dualTarget);
            this.mLabelContainer.setLongClickable(dualTarget);
        }
