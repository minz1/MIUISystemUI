...
public class AssistOrbView extends FrameLayout {
    ...
    public AssistOrbView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        this.mOffsetUpdateListener = new ValueAnimator.AnimatorUpdateListener() {
            public void onAnimationUpdate(ValueAnimator animation) {
                float unused = AssistOrbView.this.mOffset = ((Float) animation.getAnimatedValue()).floatValue();
    ...
    public void updateElevation() {
        setElevation(((float) this.mMaxElevation) * (1.0f - Math.max((((float) this.mStaticOffset) - this.mOffset) / ((float) this.mStaticOffset), 0.0f)));
    }
    ...
    private void updateLogo() {
        ...
        float translationY = ((((float) (this.mCircleRect.top + this.mCircleRect.bottom)) / 2.0f) - (((float) this.mLogo.getHeight()) / 2.0f)) - (((float) this.mCircleMinSize) / 7.0f);
        float t = (((float) this.mStaticOffset) - this.mOffset) / ((float) this.mStaticOffset);
        float translationY2 = translationY + (((float) this.mStaticOffset) * t * 0.1f);
