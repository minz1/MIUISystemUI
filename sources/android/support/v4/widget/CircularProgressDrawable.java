...
public class CircularProgressDrawable extends Drawable implements Animatable {
    ...
    private void applyFinishTranslation(float interpolatedTime, Ring ring) {
        ...
        ring.setRotation(ring.getStartingRotation() + ((((float) (Math.floor((double) (ring.getStartingRotation() / 0.8f)) + 1.0d)) - ring.getStartingRotation()) * interpolatedTime));
    }
    ...
    public void applyTransformation(float interpolatedTime, Ring ring, boolean lastFrame) {
        ...
        if (this.mFinishing) {
            ...
            if (interpolatedTime < 0.5f) {
                ...
                startTrim = (0.79f * MATERIAL_INTERPOLATOR.getInterpolation(interpolatedTime / 0.5f)) + 0.01f + startTrim3;
            } else {
                startTrim = ring.getStartingStartTrim() + 0.79f;
                startTrim2 = startTrim - ((0.79f * (1.0f - MATERIAL_INTERPOLATOR.getInterpolation((interpolatedTime - 0.5f) / 0.5f))) + 0.01f);
            }
            ring.setStartTrim(startTrim2);
            ring.setEndTrim(startTrim);
            ring.setRotation((0.20999998f * interpolatedTime) + startingRotation);
            setRotation(216.0f * (this.mRotationCount + interpolatedTime));
        }
    }
